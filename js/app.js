...
function wireUiButtons() {
    ...
    Object.keys(mapping).forEach(id => {
        ...
        if (el && typeof window[fnName] === 'function') {
            ...
            // Use onclick assignment to ensure single handler prevents duplicates from re-wiring
            el.onclick = window[fnName];
        }
    });
...
if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', () => {
        try {
            // Do NOT auto-initialize the full app on page load. Initialization should occur after
            // a successful login or explicit checkAuth(), to avoid showing the main UI to
            // unauthenticated users. We still wire UI buttons so login/logout work.
            if (typeof wireUiButtons === 'function') wireUiButtons();
